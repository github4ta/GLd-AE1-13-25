package by.vek.ui.pages.home;

public class HomeLocator {

    public static final String BUTTON_ACCEPT_COOKIES = "//button[@class='Button-module__button Button-module__blue-primary']";
    public static final String BUTTON_ACCOUNT = "//button[@class='styles_userToolsToggler__c2aHe']";
    public static final String BUTTON_LOGIN = "//button[@data-testid='loginButton']";
    public static final String BUTTON_CART = "//a[@class='headerCartBox headerCartBoxEmpty']";
    public static final String INPUT_CATALOG_SEARCH = "//input[@id='catalogSearch']";
}
